...
class RL_sudoku:
    ...
    def evaluateMove(self, location, value, mode):
    	...
    	if (self.Question[location] != 0 || \
    		self.current_X[location] == self.Answer[location] || \
    		self.current_X[location] == value):
    	# 1. change question, 2. modify right answer, 3. filling same wrong answer again
    		terminate = 1
    		loc_reward = -10
    	...
    	# evaluate value network
    		# give positive reward to location network
    		loc_reward = 1
    		# really fill in changes when location is right
    		self.current_X[location] = value
    		# finish one problem
    		if (np.array_equal(self.current_X, self.Answer)): 
                terminate = 1
                val_reward = 10
                self.genMove(mode) # start new game
            # fill in a right answer
            elif (self.Answer[location] == value): 
                terminate = 0
                val_reward = 1
    ...
    def trainModel(self):
        ...
        while (global_t < self.MAX_STEP):
            ...
            if (random.random() <= epsilon):
            	location[np.random.randrange(self.LOCATIONS)] = 1
            	value[np.random.randrange(self.VALUES)] = 1
            else:
            ...
            if (self.memory.shape[0] > self.OBSERVATION):
                ...
                mem_loc_reward, \
                mem_val_reward, \
                mem_terminate] = minibatch[:, self.INPUT_SIZE*2:]
